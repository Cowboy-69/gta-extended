...
CPed::InflictDamage(CEntity *damagedBy, eWeaponType method, float damage, ePedPieceTypes pedPiece, uint8 direction)
{
	...
	if (detectDieAnim) {
		switch (method) {
			...
			case WEAPONTYPE_M16:
#endif
#ifdef EX_WEAPON_SHOTGUN2 // CPed::InflictDamage
			case WEAPONTYPE_SHOTGUN2:
#endif
#ifdef EX_WEAPON_UZIOLD // CPed::InflictDamage
...
#endif
#ifdef EX_WEAPON_STEYR // CPed::InflictDamage
...
#ifdef EX_PED_ANIMS_IN_CAR // When a ped dies in the car, the death animation in the car will be played
					if (m_pMyVehicle->IsBoat() && m_pMyVehicle->CanPedExitCar(true)) {
#else
