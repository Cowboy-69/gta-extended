					...
					if (ms_RadarTrace[blipId].m_eRadarSprite == RADAR_SPRITE_BOMB || ms_RadarTrace[blipId].m_eRadarSprite == RADAR_SPRITE_SAVE
						...
						if (blipEntity) {
							...
							if (ms_RadarTrace[blipId].m_eBlipDisplay == BLIP_DISPLAY_BOTH || ms_RadarTrace[blipId].m_eBlipDisplay == BLIP_DISPLAY_BLIP_ONLY) {
								...
								if (!ms_RadarTrace[blipId].m_bShortRange || dist <= 1.0f || FrontEndMenuManager.bMenuMapActive)
#endif
								{
...
void CRadar::DrawGPS()
{
	...
	if (!FrontEndMenuManager.bMenuMapActive && CWorld::Players[CWorld::PlayerInFocus].m_pRemoteVehicle)
		return;

	CVehicle* vehicle = FindPlayerVehicle();
	if (!FrontEndMenuManager.bMenuMapActive && vehicle &&
		(vehicle->IsBoat() || vehicle->IsPlane() || vehicle->GetModelIndex() == MI_RCBANDIT)) {

		return;
	}

	bool bDrawRadar = CHud::m_Wants_To_Draw_Hud;
    ...
    if (FindPlayerPed() && (FindPlayerPed()->m_pMyVehicle && FindPlayerPed()->InVehicle() && bDrawRadar || FrontEndMenuManager.bMenuMapActive)) {
		// Mission blip
		if (RouteBlipId != -1 && ms_RadarTrace[RouteBlipId].m_bInUse) {
			...
			if (nodesCount > 0) {
				for (short i = 0; i < nodesCount; i++) {
					...
					if (!bDrawRadar && !FrontEndMenuManager.bMenuMapActive)
				...
				if (bDrawRadar && nodesCountDeleted != 0)
				...
				for (short i = 0; i < (nodesCount - 1); i++) {
					...
					if (!bDrawRadar && !FrontEndMenuManager.bMenuMapActive) {
						...
						float mp = 1.0f;
						if (!bDrawRadar) {
							mp = FrontEndMenuManager.fMapSize - 140.0f;
							if (mp < 140.0f)
								mp = 140.0f;
							else if (mp > 960.0f)
								mp = 960.0f;
...
void
CRadar::SetBlipRoute(int32 i, bool bEnable)
{
	if (bEnable) {
		int index = GetActualBlipArrayIndex(i);
...
#ifdef EX_SHORT_RANGE_BLIP_OPCODES
int32
CRadar::SetShortRangeCoordBlip(eBlipType type, CVector pos, uint32 color, eBlipDisplay display)
{
...
void
CRadar::SetBlipShortRange(int32 i, bool bEnable)
{
	int index = GetActualBlipArrayIndex(i);
	...
	ms_RadarTrace[index].m_bShortRange = bEnable;
}

bool
CRadar::IsBlipShortRange(int32 i)
{
	int index = GetActualBlipArrayIndex(i);
	...
	return ms_RadarTrace[index].m_bShortRange;
}
#endif
