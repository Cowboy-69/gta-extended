...
#define DEFAULT_TEXT_COLOR 235, 170, 50, 255
#define SELECTED_TEXT_COLOR 255, 217, 106, 255
#define HINT_TEXT_COLOR 255, 255, 255, 255

const int CPhotoMode::animList[] = { ANIM_STD_IDLE, ANIM_STD_IDLE_BIGGUN, ANIM_STD_IDLE_CAM, ANIM_STD_IDLE_HBHB, ANIM_STD_WALK, ANIM_STD_RUN, ANIM_STD_RUNFAST,
									 ANIM_STD_WEAPON_BAT_H, ANIM_STD_WEAPON_BAT_V, ANIM_STD_WEAPON_AK_BODY, ANIM_STD_WEAPON_PUMP, ANIM_STD_WEAPON_THROW,
									 ANIM_STD_FIGHT_PUNCH, ANIM_STD_FIGHT_HEAD, ANIM_STD_FIGHT_KICK,
									 ANIM_STD_EVADE_DIVE, ANIM_STD_GET_UP, ANIM_STD_GET_UP_FRONT, ANIM_STD_ARREST, ANIM_STD_CRAWLOUT_LHS, ANIM_STD_CAR_SIT, ANIM_STD_CAR_SIT_P };
...
void CPhotoMode::DisablePhotoMode()
{
	...
	CWeaponInfo* weaponInfo = CWeaponInfo::GetWeaponInfo((eWeaponType)FindPlayerPed()->m_currentWeapon);
	...
	if (KEYJUSTDOWN('A') || pad->IsAffectedByController && pad->GetDPadLeftJustDown()) {
		if (currentTab == TAB_CAMERA) {
			...
			if (currentItem == ITEM_CHAR_VISIBILITY) {
				...
				int maxAnimations = sizeof(animList) / 4 - 1;
				if (animID <= -1)
				...
				if (bMuzzleFlash && animID != -1)
					CreateMuzzleFlash();
			} else if (currentItem == ITEM_CHAR_WEAPON) {
				RemovePlayerWeapon();

				if (weaponID <= WEAPONTYPE_UNARMED)
					weaponID = WEAPONTYPE_DETONATOR;
				else
					weaponID--;

				for (int i = 0; i < MAX_MUZZLE_PARTICLES; i++) {
					if (muzzleFlashParticles[i] && (weaponID <= WEAPONTYPE_BASEBALLBAT || weaponID >= WEAPONTYPE_FLAMETHROWER)) {
				...
				if (fakePlayer && weaponID > WEAPONTYPE_BASEBALLBAT && weaponID < WEAPONTYPE_FLAMETHROWER)
		...
		if (currentTab == TAB_CAMERA) {
			...
			if (currentItem == ITEM_CHAR_VISIBILITY) {
				...
				if (weaponID >= WEAPONTYPE_DETONATOR)
					weaponID = WEAPONTYPE_UNARMED;
				else
					weaponID++;
...
void CPhotoMode::UpdateMuzzleParticle()
{
	if (!bMuzzleFlash)
		return;
...
char* CPhotoMode::GetTabName(int tab)
{
	...
	switch (tab) {
		case TAB_CAMERA:
			sprintf(sTemp, "PM_CAM");
			break;
		case TAB_CHARACTER:
			sprintf(sTemp, "PM_CHAR");
			break;
		case TAB_SKY:
			sprintf(sTemp, "PM_SKY");
			break;
		case TAB_LIGHT:
			sprintf(sTemp, "PM_LGHT");
			break;
		case TAB_EFFECTS:
			sprintf(sTemp, "PM_EFFE");
			break;
		default:
			sprintf(sTemp, "");
			break;
	...
	if (currentTab == TAB_CAMERA) {
		switch (item) {
			case ITEM_CAMERA_FOV:
				sprintf(sTemp, "PM_ZOOM");
				break;
			case ITEM_CAMERA_TILT:
				sprintf(sTemp, "PM_TILT");
				break;
			case ITEM_CAMERA_NEAR_CLIP:
				sprintf(sTemp, "PM_NCP");
				break;
			default:
				sprintf(sTemp, "");
				break;
		...
		switch (item) {
			case ITEM_CHAR_VISIBILITY:
				sprintf(sTemp, "PM_VSBL");
				break;
			case ITEM_CHAR_ANIMATION:
				sprintf(sTemp, "PM_ANIM");
				break;
			case ITEM_CHAR_ANIMATION_FRAME:
				sprintf(sTemp, "PM_ANFR");
				break;
			case ITEM_CHAR_WEAPON:
				sprintf(sTemp, "PM_WEAP");
				break;
			case ITEM_CHAR_MUZZLE_FLASH:
				sprintf(sTemp, "PM_MUZL");
				break;
			case ITEM_CHAR_ROTATE:
				sprintf(sTemp, "PM_ROT");
				break;
			default:
				sprintf(sTemp, "");
				break;
		...
		switch (item) {
			case ITEM_SKY_SUN_MOON:
				sprintf(sTemp, "PM_SUN");
				break;
			case ITEM_SKY_HOURS:
				sprintf(sTemp, "PM_HOUR");
				break;
			case ITEM_SKY_MINUTES:
				sprintf(sTemp, "PM_MIN");
				break;
			case ITEM_SKY_WEATHER:
				sprintf(sTemp, "PM_WTHR");
				break;
			default:
				sprintf(sTemp, "");
				break;
		...
		switch (item) {
			case ITEM_LIGHT_DEFAULT_LIGHT:
				sprintf(sTemp, "PM_DEFL");
				break;
			case ITEM_LIGHT_COLOR_RED:
				sprintf(sTemp, "PM_COLR");
				break;
			case ITEM_LIGHT_COLOR_GREEN:
				sprintf(sTemp, "PM_COLG");
				break;
			case ITEM_LIGHT_COLOR_BLUE:
				sprintf(sTemp, "PM_COLB");
				break;
			case ITEM_LIGHT_DARK_LIGHT:
				sprintf(sTemp, "PM_DARK");
				break;
			default:
				sprintf(sTemp, "");
				break;
		...
		switch (item) {
			case ITEM_EFFECT_COLORFILTER:
				sprintf(sTemp, "PM_CLFL");
				break;
			case ITEM_EFFECT_INTENSITY:
				sprintf(sTemp, "PM_INTS");
				break;
			case ITEM_EFFECT_BRIGHTNESS:
				sprintf(sTemp, "PM_BRTN");
				break;
			case ITEM_EFFECT_FRAMES:
				sprintf(sTemp, "PM_FRMS");
				break;
			default:
				sprintf(sTemp, "");
				break;
...
char* CPhotoMode::GetItemValue(int item)
{
	...
	if (currentTab == TAB_CAMERA) {
		switch (item) {
			case ITEM_CAMERA_FOV:
				sprintf(sTemp, "%i", (int)cameraFOV);
				break;
			case ITEM_CAMERA_TILT:
				sprintf(sTemp, "%.1f", cameraTilt);
				break;
			case ITEM_CAMERA_NEAR_CLIP:
				sprintf(sTemp, "%.1f", cameraNearClip);
				break;
			default:
				sprintf(sTemp, "");
				break;
		...
		switch (item) {
			case ITEM_CHAR_VISIBILITY:
				if (bCharacterVisibility)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("FEM_ON")));
				else
					sprintf(sTemp, UnicodeToAscii(TheText.Get("FEM_OFF")));
				break;
			case ITEM_CHAR_ANIMATION:
				if (animID == -1)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("FEM_NON")));
				else
					sprintf(sTemp, "%i", animID + 1);
				break;
			case ITEM_CHAR_ANIMATION_FRAME:
				sprintf(sTemp, "%.2f", animFrame);
				break;
			case ITEM_CHAR_WEAPON:
				if (weaponID <= WEAPONTYPE_UNARMED)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("FEM_NON")));
				else
					sprintf(sTemp, "%i", weaponID);
				break;
			case ITEM_CHAR_MUZZLE_FLASH:
				if (bMuzzleFlash)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("FEM_ON")));
				...
				break;
			case ITEM_CHAR_ROTATE:
				sprintf(sTemp, "%i", (int)RADTODEG(rotateAngle));
				break;
			default:
				sprintf(sTemp, "");
				break;
		...
		switch (item) {
			case ITEM_SKY_SUN_MOON:
				sprintf(sTemp, "%i", (int)RADTODEG(sunAndMoonAngle));
				break;
			case ITEM_SKY_HOURS:
				sprintf(sTemp, "%i", CClock::GetHours());
				break;
			case ITEM_SKY_MINUTES:
				sprintf(sTemp, "%i", CClock::GetMinutes());
				break;
			case ITEM_SKY_WEATHER:
				if (weatherID == WEATHER_SUNNY)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("PM_SNNY")));
				else if (weatherID == WEATHER_CLOUDY)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("PM_CLDY")));
				else if (weatherID == WEATHER_RAINY)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("PM_RANY")));
				else if (weatherID == WEATHER_FOGGY)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("PM_FGGY")));
				break;
			default:
				sprintf(sTemp, "");
				break;
		...
		switch (item) {
			case ITEM_LIGHT_DEFAULT_LIGHT:
				...
				break;
			case ITEM_LIGHT_COLOR_RED:
				sprintf(sTemp, "%.1f", lightColorRed);
				break;
			case ITEM_LIGHT_COLOR_GREEN:
				sprintf(sTemp, "%.1f", lightColorGreen);
				break;
			case ITEM_LIGHT_COLOR_BLUE:
				sprintf(sTemp, "%.1f", lightColorBlue);
				break;
			case ITEM_LIGHT_DARK_LIGHT:
				if (bDarkLightEnabled)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("FEM_ON")));
		...
		switch (item) {
			case ITEM_EFFECT_COLORFILTER:
				if (CPostFX::EffectSwitch == CPostFX::POSTFX_OFF)
					...
					sprintf(sTemp, UnicodeToAscii(TheText.Get("FEM_SIM")));
				else if (CPostFX::EffectSwitch == CPostFX::POSTFX_NORMAL)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("FEM_NRM")));
				else if (CPostFX::EffectSwitch == CPostFX::POSTFX_MOBILE)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("FEM_MOB")));
				break;
			case ITEM_EFFECT_INTENSITY:
				sprintf(sTemp, "%.1f", CPostFX::Intensity);
				break;
			case ITEM_EFFECT_BRIGHTNESS:
				sprintf(sTemp, "%.1f", (float)FrontEndMenuManager.m_PrefsBrightness / 511.0f);
				break;
			case ITEM_EFFECT_FRAMES:
				if (frameEffectID == 0)
					sprintf(sTemp, UnicodeToAscii(TheText.Get("FEM_NON")));
				else
					sprintf(sTemp, "%i", frameEffectID);
				break;
			default:
				sprintf(sTemp, "");
				break;
...
void CPhotoMode::InitFakePlayer()
{
	...
	fakePlayer = new CPed(PEDTYPE_PLAYER1);
	...
	if (assocGroup == ASSOCGRP_STD && animList[animID] != ANIM_STD_EVADE_DIVE) {
...
void CPhotoMode::DrawMenuAndEffects()
{
	...
	CFont::SetCentreOn();
	CFont::SetColor(CRGBA(DEFAULT_TEXT_COLOR));
	CFont::PrintString(SCREEN_SCALE_FROM_RIGHT((endX + startX) / 2.0f), SCREEN_SCALE_FROM_BOTTOM(endY + 2.0f), TheText.Get(GetTabName(currentTab)));
	...
	for (int i = 0; i < maxItems; i++) {
		...
		CFont::PrintString(SCREEN_SCALE_FROM_RIGHT(endX - 4.0f), SCREEN_SCALE_FROM_BOTTOM(startY - offsetItemY), TheText.Get(GetItemName(i)));
